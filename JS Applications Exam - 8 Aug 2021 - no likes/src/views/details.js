...
${userLike.length ? '' : html`<a @click=${() => onLike(book._id)} class="button" href="/details/${book._id}">Like</a>` }
`;
...
export async function detailsPage(ctx){
    ...
    const isUser = ctx.user ? ctx.user._id : '';
    const likes = await getTotalLikesCountForBook(book._id);
    const userLike = await getLikeForBookFromUser(book._id,isUser);
