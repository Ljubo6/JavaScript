...
router.post(
    ...
    body('username','Username must be at least 3 characters long')
        .trim()
        .isLength({min:3}).custom(async (value,{req}) => {
    ...
    body('repeatePassword','All fields are required').trim().notEmpty(),
...
router.post('/login',isGuest(),async(req,res) => {
    try{
        ...
        const ctx = {title:'Login',error:err.message,data:{username:req.body.username}}
